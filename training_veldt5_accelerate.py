...
def main():
    ...
    if args.from_veld_pretrained is None:
        ...
        model = VELDT5Model.from_pretrained(
            args.from_veld_pretrained
        )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        if args.output_dir is not None:
            ...
            unwrapped_model.save_pretrained(
                f"{args.output_dir}_{epoch}", is_main_process=accelerator.is_main_process, save_function=accelerator.save
            )
            if accelerator.is_main_process:
                tokenizer.save_pretrained(f"{args.output_dir}_{epoch}")
                image_tokenizer.save_pretrained(f"{args.output_dir}_{epoch}")
            
            if result is not None:
                with open(os.path.join(f"{args.output_dir}_{epoch}", "all_results.json"), "w") as f:
                    json.dump(
                        {
